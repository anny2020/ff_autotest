...
期望结果：添加成功
'''
data = [{"username":"01004","passwd":"000000","hdnr":"活动内容","expect":"操作成功"}]
...
class sfqt(TestCase):
    ...
    def test_hdjl(self,data):
        print("--------------开始测试------------------")
        print("测试数据是：%s %s %s %s" % (data['username'],data['passwd'],data['hdnr'],data['expect']))
        self.sfqt.login(data['username'],data['passwd'])
        self.sfqt.add_hdjl(data['hdnr'])
